...
评估阶段由 `Evaluate` 方法负责，以 `InferenceResult` 为输入，加载对应的 EvalCase，构造 actuals 与 expecteds 两组 Invocation 列表。expecteds 默认来自 EvalSet 的 `conversation`；当 EvalCase 开启 `expectedRunnerEnabled` 时，评估阶段直接复用推理阶段已经生成好的 `ExpectedInferences`。然后按 `EvaluateConfig.EvalMetrics` 逐条执行评估器。
