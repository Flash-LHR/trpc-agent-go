...
`approval.New(reviewer, opts...)` 是一个面向工具调用的审批插件。它拦截
...
仓库里提供了一个可直接运行的完整示例：`examples/approval`。它使用真实的 `hostexec`
...
完整示例见 [examples/approval](https://github.com/trpc-group/trpc-agent-go/tree/main/examples/approval)。

说明：目前仓库内置了 Logging、GlobalInstruction、Approval 三类插件，更多插件可通过自定义插件实现。
