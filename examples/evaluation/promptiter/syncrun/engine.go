...
type syncRunConfig struct {
	...
	TeacherTemperature         float64
	BackwardInstruction        string
...
func buildPromptIterRuntime(ctx context.Context, cfg syncRunConfig) (*promptIterRuntime, error) {
	...
	teacherAgent := newTeacherAgent(judgeModel, cfg.TeacherTemperature)
	...
	backwarderOptions := make([]backwarder.Option, 0, 1)
	if instruction := strings.TrimSpace(cfg.BackwardInstruction); instruction != "" {
		backwarderOptions = append(backwarderOptions, backwarder.WithRunOptions(agent.WithInstruction(instruction)))
	}
	backwarderInstance, err := backwarder.New(ctx, backwarderRunner, backwarderOptions...)
	if err != nil {
		agentEvaluator.Close()
		closeAll()
		return nil, fmt.Errorf("create backwarder: %w", err)
	}
	aggregatorOptions := make([]aggregator.Option, 0, 1)
	if instruction := strings.TrimSpace(cfg.AggregatorInstruction); instruction != "" {
		aggregatorOptions = append(aggregatorOptions, aggregator.WithRunOptions(agent.WithInstruction(instruction)))
	}
	aggregatorInstance, err := aggregator.New(ctx, aggregatorRunner, aggregatorOptions...)
	if err != nil {
		agentEvaluator.Close()
		closeAll()
		return nil, fmt.Errorf("create aggregator: %w", err)
	}
	optimizerOptions := make([]optimizer.Option, 0, 1)
	if instruction := strings.TrimSpace(cfg.OptimizerInstruction); instruction != "" {
		optimizerOptions = append(optimizerOptions, optimizer.WithRunOptions(agent.WithInstruction(instruction)))
	}
	optimizerInstance, err := optimizer.New(ctx, optimizerRunner, optimizerOptions...)
	if err != nil {
		agentEvaluator.Close()
		closeAll()
