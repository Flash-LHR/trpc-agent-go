...
- [syncrun](./syncrun): runs the full PromptIter optimization loop directly through `engine.Run`, and can also execute repeated A/B tests or ablations through a JSON experiment config. Its initial candidate instruction intentionally stays simple so that PromptIter, rather than a strong hand-written seed, drives the gain.
