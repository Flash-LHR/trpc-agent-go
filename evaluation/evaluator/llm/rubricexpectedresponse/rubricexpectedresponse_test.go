...
func TestRubricExpectedResponseEvaluatorDelegates(t *testing.T) {
	...
	impl, ok := ev.(*rubricExpectedResponseEvaluator)
	require.True(t, ok)
	...
	assert.Equal(t, "llm_rubric_expected_response", impl.Name())
	assert.Equal(t, "LLM rubric response evaluator against expected references", impl.Description())
