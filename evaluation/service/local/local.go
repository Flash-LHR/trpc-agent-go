...
func (s *local) inferExpectedsForEval(
	...
	if err != nil {
		err = fmt.Errorf("inference eval case (evalCaseID=%s, sessionID=%s): %w", evalCase.EvalID, inferenceResult.SessionID, err)
		return nil, err
	}
	expecteds, err := inference.Inference(
		...
		[]agent.RunOption{agent.WithInjectedContextMessages(seedMessages)},
	)
	if err != nil {
